...
RSpec.describe 'get meals endpoint' do
  ...
  it "returns all meals for a user on the given date" do
    ...
    expect(first_meal[:id]).to eq(@meal1.id)
    ...
    first_food = first_meal[:relationships][:food_entries][:data].first
  ...
  it "returns an error if the user doesn't exist" do
    get "/api/v1/users/#{@user.id + 20}/meals", params: {date: '2021-11-06T00:14:46+00:00'}
